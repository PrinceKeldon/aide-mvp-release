...
class SetupNamesRequest(BaseModel):
...
class SetupCalendarIcsRequest(BaseModel):
    source_name: str = "local"
    ics_path: str
    label: str = "Local Calendar"
    timezone: str = "UTC"


class SetupGoogleCalendarRequest(BaseModel):
...
class SetupBriefTimeRequest(BaseModel):
    hour: int
...
async def _ask_aide_about_item(item: dict, question: str | None) -> str:
    ...
    prompt = (
        "You are Vera helping the owner act on a specific Your Day item.\n"
...
async def fit_genie_page():
    return HTMLResponse(content=COMING_LATER_HTML.replace(
        "{{FEATURE_NAME}}", "Fit Genie"
    ))
...
async def setup_save(req: SetupSaveRequest):
    ...
    result = write_setup_values(values)
    return {"ok": True, "written": result["written"]}


@app.post("/api/setup/email")
async def setup_email(req: SetupEmailRequest):
    if "@" not in req.email_address or not req.email_password.strip():
        raise HTTPException(status_code=422, detail="Email address and app password are required.")
    if not (1 <= req.imap_port <= 65535 and 1 <= req.smtp_port <= 65535):
        raise HTTPException(status_code=422, detail="Email ports must be valid TCP ports.")

    account = normalize_setup_name(req.account_name, "primary")
    values = {
        f"EMAIL_ACCOUNT_{account}_ADDRESS": req.email_address.strip(),
        f"EMAIL_ACCOUNT_{account}_PASSWORD": req.email_password.strip(),
        f"EMAIL_ACCOUNT_{account}_IMAP_HOST": req.imap_host.strip() or "imap.gmail.com",
        f"EMAIL_ACCOUNT_{account}_IMAP_PORT": str(req.imap_port),
        f"EMAIL_ACCOUNT_{account}_SMTP_HOST": req.smtp_host.strip() or "smtp.gmail.com",
        f"EMAIL_ACCOUNT_{account}_SMTP_PORT": str(req.smtp_port),
        f"EMAIL_ACCOUNT_{account}_ALIASES": req.aliases.strip(),
    }
    if req.make_default:
        values["DEFAULT_EMAIL_ACCOUNT"] = account.lower()

    result = write_setup_values(values)
    return {"ok": True, "account": account.lower(), "written": result["written"]}


@app.post("/api/setup/calendar/ics")
async def setup_calendar_ics(req: SetupCalendarIcsRequest):
    path = Path(req.ics_path).expanduser()
    if not req.ics_path.strip():
        raise HTTPException(status_code=422, detail="Calendar .ics path is required.")
    if not path.exists():
        raise HTTPException(status_code=422, detail=f"Calendar file not found: {path}")

    source = normalize_setup_name(req.source_name, "local")
    values = {
        f"CALENDAR_ACCOUNT_{source}_ICS_PATH": str(path),
        f"CALENDAR_ACCOUNT_{source}_LABEL": req.label.strip() or "Local Calendar",
        f"CALENDAR_ACCOUNT_{source}_TIMEZONE": req.timezone.strip() or "UTC",
    }
    result = write_setup_values(values)
    return {"ok": True, "source": source.lower(), "written": result["written"]}


@app.post("/api/setup/calendar/google")
async def setup_calendar_google(req: SetupGoogleCalendarRequest):
    source = "GOOGLE_PRIMARY"
    values = {
        f"CALENDAR_ACCOUNT_{source}_PROVIDER": "google",
        f"CALENDAR_ACCOUNT_{source}_GOOGLE_CALENDAR_ID": req.calendar_id.strip() or "primary",
        f"CALENDAR_ACCOUNT_{source}_LABEL": req.label.strip() or "Google Calendar",
        f"CALENDAR_ACCOUNT_{source}_TIMEZONE": req.timezone.strip() or "UTC",
    }
    if req.credentials_path.strip():
        values["GOOGLE_CALENDAR_CREDENTIALS_PATH"] = req.credentials_path.strip()
    if req.token_path.strip():
        values["GOOGLE_CALENDAR_TOKEN_PATH"] = req.token_path.strip()

    result = write_setup_values(values)
    return {"ok": True, "source": source.lower(), "written": result["written"]}


@app.post("/api/setup/calendar/google/authorize")
async def setup_calendar_google_authorize(req: SetupGoogleCalendarRequest):
    try:
        from daily_brief.google_calendar import authorize_google_calendar, list_google_calendars

        token_path = authorize_google_calendar(
            credentials_path=req.credentials_path.strip() or None,
            token_path=req.token_path.strip() or None,
        )
    ...
    return {"ok": True, "token_path": str(token_path), "calendars": calendars}


@app.post("/api/setup/brief-time")
async def setup_brief_time(req: SetupBriefTimeRequest):
    if not 0 <= req.hour <= 23:
        raise HTTPException(status_code=422, detail="Hour must be between 0 and 23.")
    result = write_setup_values({"MORNING_BRIEF_HOUR": str(req.hour)})
    return {"ok": True, "hour": req.hour, "written": result["written"]}


@app.post("/api/setup/names")
async def setup_names(req: SetupNamesRequest):
    save_display_names(req.user_display_name, req.operator_name)
    return {"ok": True}
...
<header>
  ...
  <h1 id="agent-title">Vera</h1>
  <span id="status-text">connecting...</span>
  <button class="header-link" type="button" onclick="refreshChat()">Refresh Chat</button>
  <a class="header-link" href="/your-day">Your Day</a>
  <a class="header-link" href="/setup">Setup</a>
</header>
<div class="offline-banner" id="offline-banner">
  No internet — Vera is running locally on your device
</div>
<div id="messages">
  <div class="msg aide" id="welcome-message">Hi. I'm Vera, running entirely on your device. How can I help?</div>
</div>
<div id="input-area">
  ...
  <textarea id="input" placeholder="Message Vera..." rows="1"></textarea>
 ...
 <script>
   ...
   const chatHistoryKey = 'vera_desktop_chat_history_v1';
   ...
   async function checkStatus() {
     try {
       ...
       if (welcome && !window.localStorage.getItem(chatHistoryKey)) {
   ...
   function writeChatHistory(history) {
     const trimmed = history
       .filter(m => String(m?.text || '').trim() && m.type !== 'thinking')
       .slice(-80);
     window.localStorage.setItem(chatHistoryKey, JSON.stringify(trimmed));
   }
   ...
   function persistMessage(text, type) {
     ...
     history.push({text: cleanText, type});
   ...
   function refreshChat() {
     ...
     const mEl = document.getElementById('messages');
     if (!mEl) return;
     const agentName = document.getElementById('agent-title')?.textContent || 'Vera';
     mEl.innerHTML = '';
     const welcome = document.createElement('div');
     welcome.className = 'msg aide';
     welcome.id = 'welcome-message';
     welcome.textContent = `Hi. I'm ${agentName}, running entirely on your device. How can I help?`;
     mEl.appendChild(welcome);
   }
   ...
   async function sendMessage() {
     ...
     try {
       ...
       thinking.remove();
       const replyEl = addMessage('', 'aide', false);
       let buffer = '';
       ...
       persistMessage(replyEl.textContent, 'aide');
     } catch(e) {
...
COMING_LATER_HTML = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{{FEATURE_NAME}} - AIDE</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body {
    font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
  ...
  p { color: #666; font-size: 14px; line-height: 1.6; margin-bottom: 22px; }
  a {
...
<body>
  <div class="card">
    <h1>{{FEATURE_NAME}} is coming later</h1>
    <p>
      This feature is still being built and isn't part of AIDE yet.
...
<style>
  :root {
    ...
    --bg-card: #0E1D32;
    --accent: #3D8EFF;
    --accent-soft: rgba(61, 142, 255, 0.12);
    --accent-glow: rgba(61, 142, 255, 0.35);
    --text: #F4F7FB;
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body {
    ...
    background: var(--bg);
    background-image:
      radial-gradient(circle at 20% 20%, rgba(61, 142, 255, 0.06) 0%, transparent 45%),
      radial-gradient(circle at 80% 70%, rgba(61, 142, 255, 0.04) 0%, transparent 45%);
    color: var(--text);
    min-height: 100vh;
    display: flex;
    align-items: center;
  ...
  .step-track .tick.active { background: var(--accent); }
  .step-track .tick.done { background: var(--text-muted); }
  ...
  @keyframes breathe {
    0%, 100% { box-shadow: 0 0 0 0 var(--accent-glow); opacity: 0.6; transform: scale(0.9); }
    50% { box-shadow: 0 0 0 14px transparent; opacity: 1; transform: scale(1); }
  }
  .welcome-line {
  ...
  .welcome-line .accent { color: var(--accent); }
  ...
  input.name-input { font-family: 'Inter', sans-serif; }
  ...
  .ready-row span:last-child { color: var(--text-secondary); text-align: right; }
  .ready-row.ok span:last-child { color: var(--success); }
  .ready-row.warn span:last-child { color: var(--text-muted); }
  ...
  .choice-btn:hover { border-color: var(--text-muted); }
  .choice-btn.selected {
    border-color: var(--accent);
    color: var(--text);
  ...
  button.secondary:hover { color: var(--text-secondary); }
  ...
  .status-line.checking { color: var(--text-muted); }

  .done-stage { text-align: center; }
  .done-mark {
  ...
  .footer-note a { color: var(--text-secondary); text-decoration: none; }
...
<div class="stage">
  <div class="panel">
    ...
    <div class="step-track" id="step-track">
      ...
      <div class="tick" data-step="4"></div>
      <div class="tick" data-step="5"></div>
      <div class="tick" data-step="6"></div>
    </div>

    <div class="screen" id="screen-1">
      <div class="welcome-stage">
        <div class="breathing-dot"></div>
        <div class="welcome-line">She's already <span class="accent">here</span>.<br>She just needs her keys.</div>
        <div class="actions">
          <button class="primary" onclick="goScreen(2)">Continue</button>
        </div>
      </div>
    </div>

    <div class="screen" id="screen-2" style="display:none;">
      <h2 class="screen-title">Give her a mind to think with</h2>
      <p class="screen-sub">AIDE routes every request through a language model. Groq is free and ready in under a minute, or point her at an Ollama instance already running on this machine.</p>

      <div class="choice-row">
        <div class="choice-btn selected" id="choice-groq" onclick="selectBrain('groq')">Groq — free key</div>
        <div class="choice-btn" id="choice-ollama" onclick="selectBrain('ollama')">Ollama — local</div>
      </div>

      <div id="groq-fields">
        <label>API key</label>
        <input type="password" id="groq_api_key" placeholder="gsk_..." autocomplete="off">
        <div class="hint">
          No card required. <a href="https://console.groq.com/keys" target="_blank">console.groq.com/keys</a> &rarr; Create API Key &rarr; paste it here.
        </div>
      </div>

      <div id="ollama-fields" style="display:none;">
        <div class="hint" style="margin-top:0;">
          AIDE will reach for <code>http://localhost:11434</code> automatically. Make sure Ollama is running, then continue — we'll check the connection live.
        </div>
      </div>

      <div class="status-line" id="step2-status"></div>

      <div class="actions">
        <button class="secondary" onclick="goScreen(1)">Back</button>
        <button class="primary" id="step2-continue" onclick="saveStep2()">Continue</button>
      </div>
    </div>

    <div class="screen" id="screen-3" style="display:none;">
      <h2 class="screen-title">Reach her from anywhere</h2>
      <p class="screen-sub">Desktop VERA works on her own. Telegram is strongly recommended because it gives the owner mesh a mobile owner console for approvals, delegated node work, and household or enterprise setups.</p>

      <label>Bot token</label>
      <input type="password" id="telegram_bot_token" placeholder="123456789:AAAA..." autocomplete="off">
      <div class="hint">
        Message <a href="https://t.me/botfather" target="_blank">@BotFather</a> on Telegram &rarr; <code>/newbot</code> &rarr; follow the prompts &rarr; paste the token here.
      </div>

      <div class="status-line" id="step3-status"></div>

      <div class="actions">
        <button class="secondary" onclick="goScreen(4, true)">Skip, desktop only</button>
        <button class="primary" onclick="saveStep3()">Continue</button>
      </div>
    </div>

    <div class="screen" id="screen-4" style="display:none;">
      <h2 class="screen-title">Prepare Your Day before you wake up</h2>
      <p class="screen-sub">Connect email so VERA can read unread messages, draft replies, and flag follow-ups. She will not send anything without approval.</p>

      <label>Email address</label>
      <input type="text" id="email_address" placeholder="you@gmail.com" autocomplete="off">
      <div class="hint">
        Gmail users need an app password, not the account password. <a href="https://myaccount.google.com/apppasswords" target="_blank">Create Gmail app password</a>. Also enable IMAP in Gmail settings.
      </div>

      <label>App password</label>
      <input type="password" id="email_password" placeholder="App password" autocomplete="off">

      <div class="field-grid">
        <div>
          <label>IMAP host</label>
          <input type="text" id="imap_host" value="imap.gmail.com">
        </div>
        <div>
          <label>SMTP host</label>
          <input type="text" id="smtp_host" value="smtp.gmail.com">
        </div>
      </div>

      <div class="status-line" id="step4-status"></div>

      <div class="actions">
        <button class="secondary" onclick="goScreen(5, true)">Skip email</button>
        <button class="primary" onclick="saveEmailSetup()">Save email</button>
      </div>
    </div>

    <div class="screen" id="screen-5" style="display:none;">
      <h2 class="screen-title">Connect your calendar</h2>
      <p class="screen-sub">VERA uses calendar access to build the morning agenda and spot conflicts. Start with an exported .ics file, or save Google Calendar settings and authorize locally.</p>

      <div class="choice-row">
        <div class="choice-btn selected" id="choice-ics" onclick="selectCalendar('ics')">.ics file</div>
        <div class="choice-btn" id="choice-google" onclick="selectCalendar('google')">Google Calendar</div>
      </div>

      <div id="ics-fields">
        <label>.ics file path</label>
        <input type="text" id="calendar_ics_path" placeholder="/Users/you/calendar.ics">
        <div class="hint">Apple Calendar and most calendar apps can export an <code>.ics</code> file. Paste the local file path here.</div>
        <label>Calendar label</label>
        <input type="text" id="calendar_label" value="Personal Calendar">
      </div>

      <div id="google-fields" style="display:none;">
        <label>OAuth credentials JSON path</label>
        <input type="text" id="google_credentials_path" placeholder="~/.aide/data/google/calendar_credentials.json">
        <div class="hint">
          Create an OAuth Desktop Client in Google Cloud, download the credentials JSON, then authorize. CLI fallback: <code>./venv/bin/python google_calendar_auth.py</code>.
        </div>
        <label>Calendar ID</label>
        <input type="text" id="google_calendar_id" value="primary">
        <div class="actions" style="margin-top:14px;">
          <button class="secondary" onclick="authorizeGoogleCalendar()">Authorize and list calendars</button>
        </div>
      </div>

      <label>Timezone</label>
      <input type="text" id="calendar_timezone" value="Europe/Berlin">

      <div class="status-line" id="step5-status"></div>

      <div class="actions">
        <button class="secondary" onclick="goScreen(6, true)">Skip calendar</button>
        <button class="primary" onclick="saveCalendarSetup()">Save calendar</button>
      </div>
    </div>

    <div class="screen" id="screen-6" style="display:none;">
      <h2 class="screen-title">What should she call you?</h2>
      <p class="screen-sub">Entirely optional. She'll work just as well without a name on file.</p>

      <label>Your name</label>
      <input type="text" class="name-input" id="user_display_name" placeholder="Frank">

      <label>Her name</label>
      <input type="text" class="name-input" id="operator_name" placeholder="AIDE">

      <div class="actions">
        <button class="secondary" onclick="finishSetup(true)">Skip</button>
        <button class="primary" onclick="finishSetup(false)">Finish</button>
      </div>
    </div>

    <div class="screen done-stage" id="screen-done" style="display:none;">
      <div class="done-mark">&check;</div>
      <h2 class="screen-title">Ready.</h2>
      <p class="screen-sub" id="done-message">She's set up and waiting in chat.</p>
      <label>Morning brief time</label>
      <select id="brief_hour" onchange="saveBriefTime()">
      ...
      <div class="actions">
        <button class="primary" onclick="window.location.href='/'">Open chat</button>
      </div>
    </div>

    <div class="footer-note">
      <a href="/" id="skip-all-link">Skip setup, go to chat</a>
    </div>
...
<script>
let selectedBrain = 'groq';
let selectedCalendar = 'ics';
const totalSteps = 6;
...
function updateTrack(n) {
  document.querySelectorAll('.tick').forEach(t => {
    ...
    if (step < n) t.classList.add('done');
    if (step === n) t.classList.add('active');
  });
}

function selectCalendar(which) {
  selectedCalendar = which;
  document.getElementById('choice-ics').classList.toggle('selected', which === 'ics');
  document.getElementById('choice-google').classList.toggle('selected', which === 'google');
  document.getElementById('ics-fields').style.display = which === 'ics' ? 'block' : 'none';
  document.getElementById('google-fields').style.display = which === 'google' ? 'block' : 'none';
}

function goScreen(n, skippedPrevious) {
  document.querySelectorAll('.screen').forEach(el => el.style.display = 'none');
  const target = n > totalSteps ? 'screen-done' : 'screen-' + n;
  document.getElementById(target).style.display = 'block';
  if (n <= totalSteps) updateTrack(n);
  document.getElementById('skip-all-link').style.display = (n > totalSteps) ? 'none' : 'block';
}

function selectBrain(which) {
  selectedBrain = which;
  document.getElementById('choice-groq').classList.toggle('selected', which === 'groq');
  document.getElementById('choice-ollama').classList.toggle('selected', which === 'ollama');
  document.getElementById('groq-fields').style.display = which === 'groq' ? 'block' : 'none';
  document.getElementById('ollama-fields').style.display = which === 'ollama' ? 'block' : 'none';
}
...
async function saveStep2() {
  ...
  if (selectedBrain === 'groq') {
    const key = document.getElementById('groq_api_key').value.trim();
    if (!key) {
      setStatus('step2-status', 'Paste your Groq key, or switch to Ollama.', 'error');
      return;
    }
    try {
      ...
      setStatus('step2-status', 'Could not save — try again.', 'error');
    }
  } else {
    btn.disabled = true;
    setStatus('step2-status', 'Checking for Ollama...', 'checking');
    try {
      const res = await fetch('/api/setup/ollama-check');
      const data = await res.json();
      if (data.ollama_reachable) {
        goScreen(3);
      } else {
        setStatus('step2-status', "Couldn't reach Ollama at localhost:11434 — start it and try again.", 'error');
      }
    } catch (e) {
      setStatus('step2-status', 'Connection check failed — try again.', 'error');
    } finally {
      btn.disabled = false;
    }
  }
...
async function saveStep3() {
  ...
  try {
    ...
    setStatus('step3-status', 'Could not save — try again, or skip.', 'error');
  }
}
...
async function saveEmailSetup() {
  ...
  if (!email || !password) {
    setStatus('step4-status', 'Add an email address and app password, or skip this for now.', 'error');
    return;
  }
  try {
    const res = await fetch('/api/setup/email', {
      ...
      body: JSON.stringify({
        ...
        imap_host: document.getElementById('imap_host').value.trim() || 'imap.gmail.com',
        smtp_host: document.getElementById('smtp_host').value.trim() || 'smtp.gmail.com',
        aliases: 'personal,gmail',
        make_default: true
      })
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || 'Could not save email settings.');
    }
    goScreen(5);
  } catch (e) {
    setStatus('step4-status', e.message || 'Could not save email settings.', 'error');
  }
}
...
async function saveCalendarSetup() {
  try {
    let res;
    const timezone = document.getElementById('calendar_timezone').value.trim() || 'UTC';
    if (selectedCalendar === 'ics') {
      const icsPath = document.getElementById('calendar_ics_path').value.trim();
      if (!icsPath) {
        setStatus('step5-status', 'Paste a local .ics path, or skip calendar for now.', 'error');
        return;
      }
      res = await fetch('/api/setup/calendar/ics', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
          source_name: 'personal',
          ics_path: icsPath,
          label: document.getElementById('calendar_label').value.trim() || 'Personal Calendar',
          timezone
        })
      });
    } else {
      res = await fetch('/api/setup/calendar/google', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
          credentials_path: document.getElementById('google_credentials_path').value.trim(),
          calendar_id: document.getElementById('google_calendar_id').value.trim() || 'primary',
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || 'Could not save calendar settings.');
    }
    goScreen(6);
  } catch (e) {
    setStatus('step5-status', e.message || 'Could not save calendar settings.', 'error');
  }
}

async function authorizeGoogleCalendar() {
  setStatus('step5-status', 'Starting Google authorization...', 'checking');
  try {
    const res = await fetch('/api/setup/calendar/google/authorize', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        credentials_path: document.getElementById('google_credentials_path').value.trim(),
        calendar_id: document.getElementById('google_calendar_id').value.trim() || 'primary',
        label: 'Google Calendar',
        timezone: document.getElementById('calendar_timezone').value.trim() || 'UTC'
      })
    });
    const payload = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(payload.detail || 'Google authorization failed.');
    const names = (payload.calendars || []).slice(0, 4).map(c => c.summary || c.id).join(', ');
    setStatus('step5-status', names ? `Authorized. Calendars found: ${names}` : 'Authorized. No calendars listed.', '');
  } catch (e) {
    setStatus('step5-status', e.message || 'Google authorization failed. Use the CLI fallback shown above.', 'error');
  }
}

async function saveBriefTime() {
  const hour = parseInt(document.getElementById('brief_hour').value, 10);
  try {
    await fetch('/api/setup/brief-time', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
...
async function refreshReadiness() {
  try {
    ...
    const rows = [
      ['Brain', status.setup_complete ? 'Ready' : 'Needs Groq or Ollama', status.setup_complete],
      ['Owner console', status.telegram_configured ? 'Telegram connected' : 'Desktop only', status.telegram_configured],
      ['Email', (status.email_accounts || []).length ? `${status.email_accounts.length} account connected` : 'Not connected', (status.email_accounts || []).length],
      ['Calendar', (status.calendar_sources || []).length ? `${status.calendar_sources.length} source connected` : 'Not connected', (status.calendar_sources || []).length],
      ['Morning brief', `${status.morning_brief_hour || '7'}:00`, true],
    ];
    document.getElementById('readiness-list').innerHTML = rows.map(([label, value, ok]) => (
      `<div class="ready-row ${ok ? 'ok' : 'warn'}"><span>${label}</span><span>${value}</span></div>`
    )).join('');
    if (status.morning_brief_hour) {
      document.getElementById('brief_hour').value = status.morning_brief_hour;
    }
  } catch (e) { /* fine; readiness is advisory */ }
}

async function finishSetup(skipNames) {
  const userName = skipNames ? '' : document.getElementById('user_display_name').value.trim();
  const operatorName = skipNames ? '' : document.getElementById('operator_name').value.trim();
  try {
    await fetch('/api/setup/names', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({user_display_name: userName, operator_name: operatorName})
    });
  } catch (e) { /* non-fatal -- names are cosmetic, don't block completion */ }
  document.getElementById('done-message').textContent = "VERA is waiting in chat. Your Day will get stronger as email, calendar, and Telegram are connected.";
  await refreshReadiness();
  goScreen(totalSteps + 1);
}

// Returning users who already have an LLM configured skip straight to
// the Telegram screen -- no reason to re-ask for a key that already works.
(async function checkExistingStatus() {
  try {
    ...
    if (status.setup_complete) {
      goScreen(3);
      document.getElementById('skip-all-link').style.display = 'none';
      if (status.morning_brief_hour) document.getElementById('brief_hour').value = status.morning_brief_hour;
    }
  } catch (e) { /* fine, start at screen 1 */ }
})();
</script>
  ...
  <div class="shell">
    <section class="hero">
      <div class="hero-card">
        ...
        <div class="hero-subnav-note">Vera can work across Your Day, Owner Mesh, and M-Peer from this local dashboard.</div>
      </div>
...
<script>
  ...
  function renderItem(item, actionsEnabled) {
    ...
    const askButton = `<button class="action-btn" onclick="toggleAskPanel('${escapeHtml(item.id)}')">Ask Vera</button>`;
    ...
    const askPanel = `
      <div class="ask-panel" id="ask-panel-${escapeHtml(item.id)}" style="display:none;">
        <textarea id="ask-input-${escapeHtml(item.id)}" rows="2" placeholder="Ask Vera about this item..."></textarea>
        <div class="item-actions">
  ...
  async function askAideAboutItem(itemId) {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload.detail || 'Ask Vera failed');
      }
      await loadBrief();
      showToast('Vera responded.');
    } catch (error) {
      showToast(error.message || 'Ask Vera failed.', 'error');
    }
  }
...
<style>
  ...
  * { box-sizing: border-box; min-width: 0; }
  ...
  .pairing-card, .discovery-card, .peer-card, .tasks-card { padding: 22px; min-width: 0; overflow: hidden; }
  ...
  .peer-top { display: flex; justify-content: space-between; gap: 14px; align-items: flex-start; min-width: 0; }
  .peer-top, .task-header, .section-head { min-width: 0; }
  .peer-top > div { min-width: 0; max-width: 100%; }
  .peer-top h3 { margin: 0; font-size: 22px; letter-spacing: -0.03em; }
  .meta { margin-top: 14px; display: grid; gap: 8px; color: var(--muted); font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 14px; min-width: 0; max-width: 100%; }
  .meta div {
    min-width: 0;
    max-width: 100%;
  ...
  @media (max-width: 980px) {
    ...
    .peer-top, .toggle-row, .task-header, .section-head, .binding-head {
      flex-direction: column;
      align-items: flex-start;
    }
    .task-status-strip {
