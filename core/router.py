...
class ModelRouter:
    ...
    def _load_groq_usage(self) -> int:
        ...
        try:
            usage_path = settings.memory_db_path.parent / "groq_usage.json"
    ...
    def _clean_response(self, content: str, *, strip_edges: bool = True) -> str:
        ...
        for marker in markers:
            import re
            content = re.sub(marker, "", content, flags=re.IGNORECASE | re.MULTILINE)
            ...
            try:
                ...
                if provider == "ollama":
                    ...
                    async for chunk in self._ollama_stream_chat(
                        ...
                        yield self._clean_response(chunk, strip_edges=False)
